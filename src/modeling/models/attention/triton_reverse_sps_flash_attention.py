...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
                    ...
                    temporary_key_window, persistent_key_window,  #
    ...
    # Hoist the loop-invariant token index for the sliding-window path.
    if HAS_PREDICT_BIAS:
        q_tok = offs_m // 2
    ...
    for start_n in tl.range(lo, hi, BLOCK_N, warp_specialize=warp_specialize):
        ...
        if HAS_PREDICT_BIAS:
            ...
            # Hard sliding window for the temporary (input) keys. Without this,
            # keys outside the window are NOT masked, making attention effectively
            # full-causal on those tokens.
            rel = q_tok[:, None] - k_tok[None, :]
            out_of_window = rel > temporary_key_window
            normal_bias = tl.where(out_of_window, -1.0e6, 0.0)

            k_is_predict = (offs_n_abs % 2) == 1
            # Predict keys carry no additive bias (deterministic predicts); they are
            # visible subject only to the optional persistent-key window.
            persistent_bias = 0.0
            ...
            attn_bias = tl.where(k_is_predict[None, :], persistent_bias, normal_bias)
            # Predicts always self-attend.
            is_self_predict = (offs_m[:, None] == offs_n_abs[None, :]) & k_is_predict[None, :]
            attn_bias = tl.where(is_self_predict, 0.0, attn_bias)
...
def prune_invalid_configs(configs, named_args, **kwargs):
    ...
    # compile time. The kernel asserts `BLOCK_N <= HEAD_DIM`; any candidate
    # config violating that fails compilation. We also keep the existing
    # constraints: BLOCK_M must fit the sequence, and BLOCK_M >= BLOCK_N
    # outside STAGE=1 (otherwise the diagonal block isn't a square or wider).
    return [
...
@triton.autotune(configs=list(filter(keep, ATTN_FWD_CONFIGS)), key=["N_CTX", "HEAD_DIM", "FP8_OUTPUT", "warp_specialize"],
...
def _attn_fwd(sm_scale, M,  #
    ...
    # when an entire tile is masked (e.g. all predict biases = -inf). See triton_full_flash_attention.py.
    ...
    if STAGE & 1:
        acc, l_i, m_i = _attn_fwd_inner(acc, l_i, m_i, q,  #
                                        ...
                                        temporary_key_window, persistent_key_window,  #
                                        HAS_PREDICT_BIAS, HAS_PERSISTENT_KEY_WINDOW,  #
...
_REVERSE_SPS_BWD_DKDV_CONFIGS = [
    triton.Config({"BLOCK_Q": bq}, num_warps=w, num_stages=s)
    for bq in (16, 32, 64)
    for w in (4, 8)
    for s in (2, 3)
]
if "PYTEST_VERSION" in os.environ:
    _REVERSE_SPS_BWD_DKDV_CONFIGS = [triton.Config({"BLOCK_Q": 64}, num_warps=4, num_stages=2)]
...
if "PYTEST_VERSION" in os.environ:
    _REVERSE_SPS_BWD_DQ_CONFIGS = [triton.Config({"BLOCK_Q": 128}, num_warps=4, num_stages=2)]
...
@triton.jit
def _reverse_sps_bwd_dkdv_kernel(
    Q, K, V, sm_scale,
    DO,
    DK, DV,
    M, D,
    ...
    """Compute dK and dV for the sliding-window attention.

    Step-by-step:
    1) Recompute local attention scores (QK + window mask) for each q/k block.
    ...
    for q_blk_idx in range(num_q_blocks):
        ...
        # Step 1: recompute scores with predict bias.
        ...
        if HAS_PERSISTENT_KEY_WINDOW:
            persistent_rel = q_tok[:, None] - k_tok[None, :]
            persistent_bias = tl.where(persistent_rel <= PERSISTENT_KEY_WINDOW, 0.0, -1.0e6)
        bias = tl.where(k_is_predict[None, :], persistent_bias, normal_bias)
        # Predicts always self-attend.
        is_self_predict = (offs_q[:, None] == offs_k[None, :]) & k_is_predict[None, :]
        bias = tl.where(is_self_predict, 0.0, bias)
...
@triton.jit
def _reverse_sps_bwd_dq_kernel(
    Q, K, V, sm_scale,
    DO,
    DQ,
    M, D,
    ...
    """Kernel B: compute dQ with predict bias recomputation.
    ...
    for k_blk_idx in range(num_k_blocks):
        ...
        # Step 1: recompute block scores with the sliding-window mask.
        qk = tl.dot(q, kT)
        rel = q_tok[:, None] - k_tok[None, :]
        out_of_window = rel > TEMPORARY_KEY_WINDOW
        normal_bias = tl.where(out_of_window, -1.0e6, 0.0)
...
class _reverse_sps_sliding_attention(torch.autograd.Function):
    ...
    def forward(
        ...
        # 2) Launch Triton fused attention forward that recomputes predict bias on the fly.
        # 3) Save tensors required for exact backward recomputation (q/k/v/o/M/prefix/predict[/docs]).
    ...
    def backward(ctx, do):
        ...
        # 2) Kernel A computes dK/dV (recomputing the sliding-window mask).
        # 3) Kernel B computes dQ (same recomputation).
        ...
        PRE_BLOCK = 128
        BLOCK_K = 64  # BLOCK_Q is autotuned per kernel
        ...
        _reverse_sps_bwd_dkdv_kernel[grid_dkdv](
            q, k, v, ctx.sm_scale,
            do,
            dk, dv,
            M, delta,
        ...
        _reverse_sps_bwd_dq_kernel[grid_dq](
            q, k, v, ctx.sm_scale,
            do,
            dq,
            M, delta,
...
def reverse_sps_sliding_attention(
    q,
    k,
    v,
    sm_scale,
    ...
    sliding window of temporary_key_window tokens over the temporary (input) keys.
    Predicts always self-attend, and the persistent keys are visible subject only
    to the optional persistent-key window. Supports optional per-document masking
    via documents_idx_BxT.
    """
    return _reverse_sps_sliding_attention.apply(
        q,
        k,
        v,
        sm_scale,
